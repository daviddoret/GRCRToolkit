...
# constraints linked to the size of the min/max PERT approach
pert_cumul <- c(.05,.5,.95)

# initialize a vector of unknown random lambda values.
...
test_lambda <- c(0,.3,.5,.1)
test_fun <- function(x){
  return(
    degld(x = x, lambda = test_lambda)
  )
}
test_fun(.5)
plot_probability_density_function(fun=test_fun, x_start=-1, x_end=4)
