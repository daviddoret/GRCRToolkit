...
test_that("manipulations on vectors yield expected min", {
  ...
  expect_equal(min(apply_limit_min(x = rnorm(n=20), limit_value = 0, limit_behavior = "Replace", replace_function = rnorm)) == 0)
  })
...
test_that("replacement on dataframes yield expected number of rows", {
  ...
  df_output <- apply_limit_min(x = df_original, limit_value = 0, limit_behavior = "replace", target_column = rf)
